...
async def scrape_user_posts(username: str, page_size=12, max_pages: Optional[int] = None):
    ...
    while True:
        ...
        # Ensure user-specific results directory exists
        output_dir = Path(f"results/{username}")
        output_dir.mkdir(parents=True, exist_ok=True)

        # Save debug data per user instead of overwriting ts2.json
        with open(output_dir / f"{username}_debug.json", "w", encoding="utf-8") as f:
        ...
        for post in posts["edges"]:
            ...
            log.debug(f"{username} - Post date: {datetime.datetime.fromtimestamp(post_data.get('taken_at_timestamp', 0))}")

            if post_data.get("taken_at_timestamp", 0) < cutoff_timestamp:
                log.debug(f"Reached post from before 2024, stopping.")
                continue
            
            if post_data.get("is_video") or "video_url" in post_data or post_data.get("clips_metadata"):
                log.debug("Skipping video/Reel post")
                continue
            
            yield parse_user_posts(post_data)
